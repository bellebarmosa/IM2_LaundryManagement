...
const AdminSettings = () => {
  ...
  useEffect(() => {
    ...
    const fetchAdminProfile = async () => {
      // Mock API call
      const response = await fetch('http://localhost:3001/user/adminProfile');
  ...
  const handleSaveChanges = () => {
    if (newPassword === confirmPassword) {
      ...
      console.log('Admin information updated:', { newUsername, newEmail, newPhone, newPassword });
      ...
      {editMode && (
        <form className="mt-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="mb-4">
            ...
            />
            {!passwordsMatch && <p className="text-red-500 text-sm mt-1">Passwords do not match. Please try again.</p>}
          </div>
          <div className="flex">
            <button
