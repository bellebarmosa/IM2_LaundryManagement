...
useEffect(() => {
  // fetch orders from the backend 
    const fetchorders = async() => {
      try{
        const response =  await Axios.get('http://localhost:3001/order/pendingorders')
              ...
              {orders && orders.map(( val, key) => {
                return <div key={ key }  className="flex flex-row items-center pb-3">
                  ...
                  <div className="flex flex-col w-1/2 pl-5">
                    <p className="text-darkBlue text-xl font-semibold">{customers && customers.find((customer) => val.customer_ID === customer.customer_ID)?.customer_name}</p>
                    <p className="text-darkBlue text-base  font-medium">{customers && customers.find((customer) => val.customer_ID === customer.customer_ID)?.customer_eMail}</p>
                  </div>
                  <p className="text-darkBlue text-xl font-semibold text-right w-1/2 pr-4">+₱{ val.order_total }</p>
                </div>
              })}
            </div>
        </div>
